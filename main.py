...
from slurm_data import SlurmDataCollector
import pandas as pd
...
class Dashboard(App):
    ...
    def __init__(self):
        super().__init__()
        self.data_collector = SlurmDataCollector()
        ...
        yield Container(
            Horizontal(
                ...
                Vertical(
                    ...
                    JobStats("Total Jobs", "0"),
                    JobStats("Active Jobs", "0"),
                    JobStats("Completed Jobs", "0"),
                    JobStats("Failed Jobs", "0"),
                    classes="stats-container"
                ),
            ),
            Horizontal(
    ...
    def update_job_stats(self) -> None:
        ...
        for stat_name, value in stats.items():
            if isinstance(value, float):
                value = f"{value:.2f}"
            self.query_one(f"JobStats[label='{stat_name}']").value = str(value)
